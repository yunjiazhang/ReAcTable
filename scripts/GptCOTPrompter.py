...
def normalize_numeric_columns(df):
    for col in df.columns:
        if df[col].dtype == object:
            ...
            # try:
            #     df[col] = df[col].apply(lambda x: eval(x.replace('e', '0')))
            #     continue
            # except:
            #     pass
                ...
                if safe and "try:" not in code and "def" in code and '(s)' in code:
                    ...
                    for line in codes:
                        if 'def' in line:
                            ...
                            add_indent = True
                        elif add_indent and not line.startswith('    '):
...
# Given the above database table DF, answer the question by executing a series of SQL queries step-by-step. At each step, choose from generating a SQL query, or directly answering the question. Question: "{}".
# """
#         self.intermediate_prompt_template = \
# """The database table DF after executing the SQL query is shown as follows:
# {}

# Continue to answer the question above: "{}". Choose from generating a SQL query, or directly answering the question.
# """
